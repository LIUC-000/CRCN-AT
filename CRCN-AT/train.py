...
def train(encoder, decoder, classifier, loss_nll, loss_mse, train_dataloader, optimizer_encoder, optimizer_decoder, optimizer_classifier, epoch, device):
    ...
    for data, target in train_dataloader:
        ...
        mse_loss_batch = lamda_re * loss_mse(re_input, Mean_data[target, :, :]) + lamda_re * loss_mse(re_input_adv, Mean_data[target, :, :])
        ...
        target_unique = torch.unique(target, dim=0)
        for c in target_unique:
            index_shot = [index for index, value in enumerate(target) if value == c]
            # index_target = random.sample(index_shot, 1)
            if d == 1:
                Mean_data[c, :, :] = data_adv[index_shot[0], :, :]
                d = 0
            else:
                Mean_data[c, :, :] = data[index_shot[0], :, :]
                d = 1
...
def TrainDataset_prepared(n_classes, k, RANDOM_SEED):
    ...
    Mean_data = DataAverage(n_classes, X_train, value_Y_train)

    return X_train, X_val, X_test, value_Y_train, value_Y_val, value_Y_test, Mean_data
...
conf = Config()
X_train, X_val, X_test, value_Y_train, value_Y_val, Y_test, Mean_data = TrainDataset_prepared(conf.n_classes, conf.k_shot, sample_seed)
