...
def train(encoder, classifier, loss_nll, train_dataloader, optimizer_encoder, optimizer_classifier, epoch, device):
    ...
    for data, target in train_dataloader:
        ...
        nll_loss_batch = 0.5 * loss_nll(output, target) + 0.5 * loss_nll(output_adv, target)
...
def train_and_test(encoder, classifier, loss_nll, train_dataloader, val_dataloader, optim_encoder, optim_classifier, epochs, save_path_encoder, save_path_classifier, device):
    current_min_val_loss = 100000000
    for epoch in range(1, epochs + 1):
        # adjust_learning_rate(optimizer, epoch)
        train(encoder, classifier, loss_nll, train_dataloader, optim_encoder, optim_classifier, epoch, device)
...
def TrainDataset_prepared(n_classes, k, RANDOM_SEED):
    X_train, X_val, X_test, value_Y_train, value_Y_val, value_Y_test = TrainDatasetKShotRound(n_classes, k, RANDOM_SEED)
...
def run(train_dataloader, val_dataloader, epochs, save_path_encoder, save_path_classifier, device):
    ...
    train_and_test(encoder, classifier, loss_nll, train_dataloader=train_dataloader, val_dataloader=val_dataloader,
                   optim_encoder=optim_encoder, optim_classifier=optim_classifier, epochs=epochs, save_path_encoder=save_path_encoder, save_path_classifier=save_path_classifier, device=device)
...
def main():
    ...
    X_train, X_val, X_test, value_Y_train, value_Y_val, Y_test = TrainDataset_prepared(conf.n_classes, conf.k_shot, sample_seed)
